...
const state = {
   ...
   bet: 5,
   spinning: false,
   isSolo: true,        // solo mode by default
   soloSpins: 0,
   soloBestWin: 0
};
...
function updateUI() {
   ...
   if (state.isSolo) {
      document.getElementById("soloCoins").textContent = state.coins[0] + " ⚡";
      document.getElementById("soloSpins").textContent = state.soloSpins;
      document.getElementById("soloBestWin").textContent = state.soloBestWin;
      document.getElementById("soloGoal").textContent = state.goalCoins;
   } else {
...
function evaluateResult() {
   ...
   if (state.isSolo) {
      state.soloSpins++;
      if (winnings > state.soloBestWin) state.soloBestWin = winnings;
   }
   ...
   setTimeout(() => {
      if (!state.isSolo) {
         ...
         if (state.bet > state.coins[0]) {
            state.bet = Math.max(1, state.coins[0]);
         }
      }
      updateUI();
   }, 1200);
...
function showEndModal(reason, playerIndex) {
   ...
   if (state.isSolo) {
      if (reason === "broke") {
         emoji = "🐔";
         title = "lêu lêu đồ gà! 🐔";
         msg   = `${n0} ran out of credits after ${state.soloSpins} spins!`;
      } else {
         emoji = "🏆";
         title = `${n0} Wins!`;
         msg   = `Reached ${state.goalCoins} ⚡ in ${state.soloSpins} spins! Best win: ${state.soloBestWin} ⚡ 🌌`;
      }
      ...
      row.className = "final-score-row winner-row";
      row.innerHTML = `<span class="final-score-name">${reason === "goal" ? "🏆 " : "⭐ "}${n0}</span><span class="final-score-coins">${c0} ⚡</span>`;
...
function startGame() {
   ...
   // Show/hide correct scoreboard
   document.getElementById("scoreboardMulti").classList.toggle("hidden", state.isSolo);
   document.getElementById("scoreboardSolo").classList.toggle("hidden", !state.isSolo);

   if (state.isSolo) {
      document.getElementById("soloNameDisplay").textContent = state.names[0];
      document.getElementById("goalDisplay").textContent = `Reach ${state.goalCoins} ⚡ to win!`;
   } else {
...
window.addEventListener("DOMContentLoaded", () => {
   ...
   document.querySelectorAll(".coin-btn:not(.mode-btn)").forEach(btn => {
      btn.addEventListener("click", () => {
         document.querySelectorAll(".coin-btn:not(.mode-btn)").forEach(b => b.classList.remove("active"));
   ...
   document.querySelectorAll(".mode-btn").forEach(btn => {
      btn.addEventListener("click", () => {
         ...
         btn.classList.add("active");
         isSolo = btn.dataset.mode === "solo";
         document.getElementById("p2Field").style.display   = isSolo ? "none" : "";
         document.getElementById("vsDivider").style.display = isSolo ? "none" : "";
         document.getElementById("p1Label").textContent     = isSolo ? "⭐ Your Name" : "⭐ Player 1";
      });
   });
